...
function publicationView(
  row: typeof schema.artifactPublication.$inferSelect,
): PublicationView {
  return {
    id: row.id,
  ...
  return {
    ...
    async findAsset(versionId, path) {
      const [row] = await database
        ...
        .innerJoin(
          ...
          eq(
            schema.contentBundleAsset.bundleId,
            schema.artifactVersion.contentBundleId,
          ),
        )
        .where(
          and(
    ...
    async findEntryAsset(versionId) {
      const [row] = await database
        ...
        .innerJoin(
          ...
          eq(
            schema.contentBundleManifest.bundleId,
            schema.artifactVersion.contentBundleId,
          ),
        )
        .innerJoin(
          ...
          and(
            ...
            eq(
              schema.contentBundleAsset.path,
              schema.contentBundleManifest.entryPath,
            ),
          ),
        )
        .where(
    ...
    async findOwnedVersionExport(ownerUserId, versionId) {
      const [version] = await database
        .select({
          ...
          artifactName: schema.artifact.name,
        })
        .from(schema.artifactVersion)
        .innerJoin(
          schema.artifact,
    ...
    async publish(input): Promise<PublishResult> {
      return database.transaction(async (transaction) => {
        const [artifact] = await transaction
          ...
          .where(
            and(
              eq(schema.artifact.id, input.artifactId),
        ...
        const existing =
          await transaction.query.artifactIdempotencyRecord.findFirst({
            where: and(
              eq(
                schema.artifactIdempotencyRecord.ownerUserId,
                input.ownerUserId,
              ),
              eq(schema.artifactIdempotencyRecord.operation, "publish"),
              eq(
                schema.artifactIdempotencyRecord.targetResourceId,
                input.artifactId,
              ),
        ...
        if (existing) {
          ...
          if (
            !existing.requestEvidence ||
            !existing.requestEvidenceKeyRevision
          ) {
            throw new Error(
              "Completed Publish idempotency record has no encrypted evidence.",
            );
          }
          ...
          if (requestEvidence !== input.requestHash)
            return { kind: "idempotency_conflict" };
          const storedPublication = existing.responseBody.publication as Record<
            string,
            unknown
          >;
          const storedLink = existing.responseBody.shareLink as Record<
            string,
            unknown
          >;
          if (
            ...
            throw new Error(
              "Completed Publish idempotency response is invalid.",
            );
          return {
            kind: "published",
            publication: {
              ...
              expirationKind:
                storedPublication.expirationKind as PublicationView["expirationKind"],
              durationSeconds:
                typeof storedPublication.durationSeconds === "number"
                  ? storedPublication.durationSeconds
                  : null,
              expiresAt:
                typeof storedPublication.expiresAt === "string"
                  ? new Date(storedPublication.expiresAt)
                  : null,
              endedAt: null,
              endReason: null,
            },
        ...
        const blocked = await transaction.execute(sql<{ blocked: boolean }>`
          select exists(
            select 1 from gallery_public_sharing_restriction where artifact_id=${input.artifactId} and state='active'
          ) or exists(
            select 1 from gallery_artifact_takedown where artifact_id=${input.artifactId} and state='active'
          ) as blocked
        `);
        if (blocked.rows[0]?.blocked) return { kind: "governance_blocked" };
        ...
        if (expiration.expiresAt !== null && expiration.expiresAt <= now)
        ...
        let link = await transaction.query.artifactShareLink.findFirst({
          where: and(
            eq(schema.artifactShareLink.artifactId, input.artifactId),
        ...
        if (!link) {
          const [createdLink] = await transaction
            .insert(schema.artifactShareLink)
            .values({
        ...
        const shareLink: ShareLinkAccessRecord = {
          shareSlug: link.slug,
          state: "active",
        };
        const responseBody = {
    ...
    async updateExpiration(ownerUserId, artifactId, publicationId, expiration) {
      return database.transaction(async (transaction) => {
        const blocked = await transaction.execute(sql<{ blocked: boolean }>`
          select exists(
            select 1 from gallery_public_sharing_restriction where artifact_id=${artifactId} and state='active'
          ) or exists(
            select 1 from gallery_artifact_takedown where artifact_id=${artifactId} and state='active'
          ) as blocked
        `);
        if (blocked.rows[0]?.blocked) return null;
        const [artifact] = await transaction
          .select({ id: schema.artifact.id })
          .from(schema.artifact)
          .where(
    ...
    async resolveShareSlug(shareSlug): Promise<ShareResolution> {
      ...
      if (!publication || publication.endReason === "unpublished")
        return { kind: "unpublished" };
      if (publication.endedAt !== null) return { kind: "unpublished" };
      if (publication.expiresAt !== null && publication.expiresAt <= new Date())
        return { kind: "expired" };
      const blocked = await database.execute(sql<{ blocked: boolean }>`
        select exists(
          select 1 from gallery_public_sharing_restriction where artifact_id=${link.artifactId} and state='active'
        ) or exists(
          select 1 from gallery_artifact_takedown where artifact_id=${link.artifactId} and state='active'
        ) as blocked
      `);
