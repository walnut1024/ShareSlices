...
export class GalleryRollbackCoordinator {
  constructor(private readonly directClients: DirectClientSource, private readonly storage?: Pick<ObjectStorage, "removeStagingPrefix">) {}

  async reconcileDisabled(): Promise<void> {
    let attemptPrefixes: string[] = [];
    await this.directClients.withClient(async (client) => {
      let committed = false;
      try {
