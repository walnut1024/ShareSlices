...
export function ArtifactsPage({ onAccepted }: { onAccepted: (artifactId: string) => void }) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (active && generation === mutationGeneration.current) setArtifacts(value);
      } catch {
  ...
  const visibleArtifacts = useMemo(() => {
    ...
    return artifacts.filter((artifact) => {
      ...
      const matchesQuery = !normalizedQuery || artifact.name.toLowerCase().includes(normalizedQuery);
  ...
  function explainUnavailable(action: "publish" | "delete"): boolean {
    ...
    const unavailable = selectedArtifacts.filter((artifact) =>
      !artifact.allowedActions.includes(action) || (action === "publish" && !artifact.readyVersion)
    );
    if (unavailable.length === 0) return false;
    const processingCount = unavailable.filter((artifact) => artifact.processingState === "accepted" || artifact.processingState === "processing").length;
    const missingReadyVersionCount = action === "publish" ? unavailable.filter((artifact) => !artifact.readyVersion && artifact.processingState === "ready").length : 0;
    const otherCount = unavailable.length - processingCount - missingReadyVersionCount;
    const reasons = [
      processingCount > 0 ? `${processingCount} ${processingCount === 1 ? "is" : "are"} still processing` : null,
      missingReadyVersionCount > 0 ? `${missingReadyVersionCount} ${missingReadyVersionCount === 1 ? "has" : "have"} no ready Version` : null,
      otherCount > 0 ? `${otherCount} ${otherCount === 1 ? "is" : "are"} not eligible` : null
    ].filter(Boolean).join("; ");
    toast.error(`${unavailable.length} selected ${unavailable.length === 1 ? "artifact" : "artifacts"} cannot be ${action === "publish" ? "published" : "deleted"} because ${reasons}.`);
    return true;
  }
  ...
  async function publishSelected(expiration: BatchExpiration, customExpiration: string) {
    ...
    setPending(true);
    const results = await mapWithConcurrency(selectedArtifacts, 3, async (artifact) => {
      const response = await publishArtifact(artifact.id, {
        versionId: artifact.readyVersion!.id,
        expiration: expiration === "permanent"
          ? { kind: "permanent" }
          : expiration === "custom"
            ? { kind: "exact", expiresAt: exactExpiration! }
            : { kind: "duration", durationSeconds: expiration === "7d" ? 604800 : 2592000 },
        link: { mode: "reuse" }
      }, crypto.randomUUID());
      if (!response || !("artifact" in response)) throw new Error("The published artifact response was incomplete.");
    ...
    setPending(false);
    if (failed.length === 0) {
      toast.success(`${succeeded.length} ${succeeded.length === 1 ? "artifact" : "artifacts"} published.`);
    ...
    setBatchPublishOpen(false);
    toast.error(`${succeeded.length} published, ${failed.length} failed. ${errorMessage(failed[0]!.reason)}`);
  }
  ...
  async function deleteSelected() {
    ...
    setArtifacts((current) => current?.filter((artifact) => !succeededIds.has(artifact.id)) ?? current);
    setPending(false);
    setBatchDeleteOpen(false);
    if (failed.length === 0) {
      toast.success(`${succeeded.length} ${succeeded.length === 1 ? "artifact" : "artifacts"} deleted.`);
      exitSelectionMode();
      return;
    }
    setSelectedIds(new Set(failed.map((result) => result.item.id)));
    toast.error(`${succeeded.length} deleted, ${failed.length} failed. ${errorMessage(failed[0]!.reason)}`);
  }
    ...
    <div className="flex min-h-[calc(100vh-110px)] flex-col">
      ...
      {!error && selectionMode ? (
        ...
        <div className="mb-[18px] flex items-center justify-between gap-4 border-b pb-3.5">
          ...
          <div className="flex items-center gap-2">
            ...
            {artifacts && artifacts.length > 0 ? <Button variant="outline" onClick={() => setSelectionMode(true)}><CheckSquare2 data-icon="inline-start" />Select</Button> : null}
            <DropdownMenu>
      ...
      {visibleArtifacts.length > 0 && view === "list" ? (
        <ArtifactList
          ...
          onPublication={(artifact) => setPublicationDialog({ artifact, mode: artifact.publicationStatus === "published" ? "manage" : "publish" })}
      ...
      <BatchPublishDialog count={selectedArtifacts.length} open={batchPublishOpen} pending={pending} onClose={() => setBatchPublishOpen(false)} onConfirm={(expiration, customExpiration) => void publishSelected(expiration, customExpiration)} />
      <BatchDeleteDialog count={selectedArtifacts.length} open={batchDeleteOpen} pending={pending} onClose={() => setBatchDeleteOpen(false)} onConfirm={() => void deleteSelected()} />
    </div>
  );
}
...
function ArtifactList({ artifacts, selectedIds, selectionMode, onSelect, onPublication, onRename, onDelete }: {
  artifacts: Artifact[];
  ...
  return (
    <div className="overflow-hidden rounded-xl border">
      <Table>
        <TableHeader><TableRow>{selectionMode ? <TableHead className="w-12"><span className="sr-only">Selected</span></TableHead> : null}<TableHead>Artifact</TableHead><TableHead className="w-44">Processing</TableHead><TableHead className="w-44">Publication</TableHead><TableHead className="w-48">Last modified</TableHead><TableHead className="w-14"><span className="sr-only">Actions</span></TableHead></TableRow></TableHeader>
        <TableBody>
          {artifacts.map((artifact) => {
            ...
            return (
              <TableRow
                ...
                onClick={(event) => { if (!selectionMode && (!(event.target instanceof Element) || !event.target.closest("a,button"))) window.location.href = detailUrl; }}
                onKeyDown={(event) => { if (!selectionMode && event.key === "Enter") window.location.href = detailUrl; }}
              >
                {selectionMode ? <TableCell onClick={(event) => event.stopPropagation()}><Checkbox aria-label={`Select ${artifact.name}`} checked={selected} onCheckedChange={() => onSelect(artifact.id)} /></TableCell> : null}
                <TableCell><a className="font-medium hover:underline" href={detailUrl}>{artifact.name}</a></TableCell>
                <TableCell className="text-muted-foreground">{processingLabel(artifact)}</TableCell>
                <TableCell><StatusBadge artifact={artifact} /></TableCell>
                <TableCell className="font-mono text-xs text-muted-foreground">{formatModified(artifact.updatedAt)}</TableCell>
                <TableCell onClick={(event) => event.stopPropagation()}>{!selectionMode ? <ArtifactMenu artifact={artifact} detailUrl={detailUrl} onPublication={() => onPublication(artifact)} onRename={() => onRename(artifact)} onDelete={() => onDelete(artifact)} /> : null}</TableCell>
              </TableRow>
            );
          })}
        </TableBody>
...
function ArtifactMenu({ artifact, detailUrl, overlay = false, onPublication, onRename, onDelete }: { artifact: Artifact; detailUrl: string; overlay?: boolean; onPublication?: () => void; onRename: () => void; onDelete: () => void }) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-44">
        <DropdownMenuGroup>
          ...
          {onPublication && artifact.readyVersion && (artifact.allowedActions.includes("publish") || artifact.allowedActions.includes("manage_publication")) ? <DropdownMenuItem onClick={onPublication}><Rocket />{artifact.publicationStatus === "published" ? "Manage publication" : "Publish"}</DropdownMenuItem> : null}
...
function BatchPublishDialog({ count, open, pending, onClose, onConfirm }: { count: number; open: boolean; pending: boolean; onClose: () => void; onConfirm: (expiration: BatchExpiration, customExpiration: string) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && !pending && onClose()}>
      <DialogContent className="sm:max-w-[480px]" showCloseButton={!pending}>
        <DialogHeader><DialogTitle>Publish {count} artifacts</DialogTitle><DialogDescription>Publish the latest ready Version of {count} selected artifacts. Existing Share links will be reused.</DialogDescription></DialogHeader>
        <FieldGroup><Field><FieldLabel htmlFor="batch-publication-expiration">Access period</FieldLabel><Select items={batchExpirationOptions} value={expiration} onValueChange={(value) => setExpiration(value as BatchExpiration)}><SelectTrigger id="batch-publication-expiration" className="w-full"><SelectValue /></SelectTrigger><SelectContent alignItemWithTrigger={false}><SelectGroup>{batchExpirationOptions.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectGroup></SelectContent></Select></Field>{expiration === "custom" ? <Field><FieldLabel htmlFor="batch-custom-expiration">Custom expiration</FieldLabel><Input id="batch-custom-expiration" type="datetime-local" value={customExpiration} onChange={(event) => setCustomExpiration(event.target.value)} /></Field> : null}</FieldGroup>
        <DialogFooter><Button type="button" variant="secondary" disabled={pending} onClick={onClose}>Cancel</Button><Button type="button" disabled={pending} onClick={() => onConfirm(expiration, customExpiration)}>{pending ? <Spinner data-icon="inline-start" /> : <Rocket data-icon="inline-start" />}Publish {count} artifacts</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}

function BatchDeleteDialog({ count, open, pending, onClose, onConfirm }: { count: number; open: boolean; pending: boolean; onClose: () => void; onConfirm: () => void }) {
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && !pending && onClose()}>
      <DialogContent className="sm:max-w-[440px]" showCloseButton={!pending}>
        <DialogHeader><DialogTitle>Delete {count} artifacts?</DialogTitle><DialogDescription>This permanently deletes the selected artifacts, including all Versions, Publications, Share links, and stored files. This cannot be undone.</DialogDescription></DialogHeader>
        <DialogFooter><Button type="button" variant="secondary" disabled={pending} onClick={onClose}>Cancel</Button><Button type="button" variant="destructive" disabled={pending} onClick={onConfirm}>{pending ? <Spinner data-icon="inline-start" /> : <Trash2 data-icon="inline-start" />}Delete {count} artifacts</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
...
function InitialEmptyState({ onAccepted }: { onAccepted: (artifactId: string) => void }) {
  const [droppedFile, setDroppedFile] = useState<File | null>(null);
  const [dragging, setDragging] = useState(false);
  return <Empty className={dragging ? "min-h-[380px] rounded-[14px] border border-dashed border-foreground/40 bg-muted/60 p-12 ring-3 ring-ring/20" : "min-h-[380px] rounded-[14px] border border-dashed bg-muted/35 p-12"} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDragging(false); }} onDrop={(event) => { event.preventDefault(); setDragging(false); setDroppedFile(event.dataTransfer.files[0] ?? null); }}><EmptyHeader><span className="mb-1 flex size-13 items-center justify-center rounded-[14px] bg-muted text-muted-foreground"><CloudUpload aria-hidden="true" /></span><EmptyTitle><h2>No artifacts yet</h2></EmptyTitle><EmptyDescription className="flex flex-col"><span>Upload your first artifact to start sharing.</span><span>Drag and drop a ZIP or self-contained HTML file here, or use the button below.</span></EmptyDescription></EmptyHeader><CreateArtifactDialog initialFile={droppedFile} onAccepted={onAccepted} /></Empty>;
}

function FilteredEmptyState({ onClear }: { onClear: () => void }) {
  return <Empty className="min-h-[320px]"><EmptyHeader><EmptyTitle><h2>No artifacts found</h2></EmptyTitle><EmptyDescription>No artifacts match the current search and filters.</EmptyDescription></EmptyHeader><Button variant="outline" onClick={onClear}>Clear search and filters</Button></Empty>;
}
...
function processingLabel(artifact: Artifact): string {
  return { accepted: "Accepted", processing: "Processing", ready: "Ready", failed: "Needs attention" }[artifact.processingState];
}
...
async function mapWithConcurrency<TItem, TValue>(items: TItem[], limit: number, operation: (item: TItem) => Promise<TValue>): Promise<ConcurrentResult<TItem, TValue>[]> {
  ...
  async function worker() {
    while (nextIndex < items.length) {
      ...
      try {
        results[index] = { status: "fulfilled", item, value: await operation(item) };
      } catch (reason) {
