...
An Artifact has at most one open Gallery listing, meaning Pending or Listed. Share to Gallery defaults to its latest ready Version but lets the Owner select any historical ready Version. It requires a public title, defaults that title from the Owner-facing Artifact name, accepts an optional description and one through five tags, shows a Version-specific Gallery cover, and requires explicit acceptance of the current Gallery permission grant. A User's first Share to Gallery also requires explicit confirmation of a public Creator display name; ShareSlices never derives or publishes that name from an email address.
