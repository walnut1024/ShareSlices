...
- `ThumbnailRenderingModule` is current in `worker/src/thumbnail.rs`. It owns the non-blocking thumbnail attempt after Version commit, including the bounded Chromium child process, fixed `1440x900` capture, animation suppression, render timeout, WebP encoding, private attempt-unique output for one Content bundle and renderer revision, and losing-attempt cleanup. It never changes Version readiness or Publication state.
