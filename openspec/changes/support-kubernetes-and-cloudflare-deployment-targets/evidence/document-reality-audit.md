...
| **High** | Cloudflare's current Hyperdrive pages are internally inconsistent about default origin certificate verification. The supported-features page says PostgreSQL `require` validates against WebPKI, while the Workers VPC guide says Hyperdrive does not verify the origin certificate by default. Existing prototype evidence proves encryption through `pg_stat_ssl`, not server identity verification. | Design verified-provider baseline and database seam; `cloudflare-deployment` database requirement; tasks 1.3, 1.4, 5.1-5.4, and 12.1. | Do not use `TLS active` or `ssl=true` as proof of authenticated origin identity. Require an explicit qualified TLS mode, preferably `verify-full` for a public origin, and a negative hostname/certificate test. For Workers VPC, record and verify its explicit certificate mode. Until the first-party contradiction is resolved by live configuration evidence, treat default-mode verification as unknown. | [Hyperdrive supported TLS modes](https://developers.cloudflare.com/hyperdrive/reference/supported-databases-and-features/), [Hyperdrive with Workers VPC](https://developers.cloudflare.com/hyperdrive/configuration/connect-to-private-database-vpc/) |
