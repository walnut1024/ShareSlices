...
| Queue delivery model | Supported | Queues provide at-least-once delivery. Messages can be duplicated; batch failures retry unacknowledged messages. Each queue has one consumer type, and a queue can have one push consumer Worker. Messages are limited to 128 KB. | The database-authoritative/wake-signal design is correct. Keep payloads to identifiers, make claims idempotent, configure retries/DLQ, and add reconciliation for DLQ and retention exhaustion. For release probes, retain a terminal nonce/sub-fence beyond the late-delivery window and drain active invocations before cross-storage final cleanup. |
...
| Worker rollback | Supported with boundaries | A rollback immediately shifts 100% traffic to a selected recent version. Connected resources are not changed, and rollback can fail when a binding resource or Durable Object history is incompatible. The Jobs path uses immediate deploy rather than version activation. | Reactivate retained App/Content versions, redeploy the retained Jobs bundle with triggers isolated, and observe Container convergence separately. Keep Terraform-owned routes, Cron, Queue consumers, R2, Queues, and Hyperdrive current and prove them compatible. |
| Terraform/Wrangler ownership | Supported but requires field-level discipline | Cloudflare provides Terraform resources for R2, Queues, routes/domains, Worker scripts/deployments, and Hyperdrive. Cloudflare warns that managing the same object through Terraform and another interface causes state drift. Hyperdrive credentials are sensitive but still exist in Terraform state. | Choose exactly one owner for every field. Terraform owns declared long-lived infrastructure; Wrangler owns App/Content versions and Jobs immediate deployment plus target-specific bindings/Secrets; the operator Secret source owns values before release mutation. Protect Terraform state as secret-bearing material. |
...
11. every synthetic verifier database row, broker attempt, Container invocation, and R2 object is release/fence/nonce-owned in a non-product namespace; marking the nonce terminal and advancing its sub-fence blocks new work, bounded active-invocation quiescence precedes final cross-storage inventory and cleanup, and the tombstone remains through the late-delivery window without being treated as an orphan.
...
The same field must never be mutated by both Terraform and Wrangler.
