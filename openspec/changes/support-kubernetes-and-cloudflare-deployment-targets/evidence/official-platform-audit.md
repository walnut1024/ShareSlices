...
| Container image/version rollout | Partially proven | A Jobs configuration can select a Container image, but `wrangler deploy` rolls Container instances separately, normally 10% then 100%; selecting a retained bundle is not proof that instances converged. The current Durable Object manual says configurations containing `exports` fail `wrangler versions upload`, gradual deployment cannot cross a Durable Object lifecycle change, and Wrangler behavior is evolving. Wrangler 4.112.0 documents JSON application and instance inventory commands; the application output includes ID, name, image, and version, while instance output includes ID, state, and application version. Provider instance IDs are runtime observations because instances are created on demand, not values available for the pre-run authorization message. | Pin and qualify the exact toolchain. Embed a pre-publication build/release/contract identity mapped to the image digest/provider reference, exercise every production-capable stable slot, then correlate each runtime-reported instance ID against the exact checkpointed application name, image, version, and instance inventory. Reject while any selectable instance has another application version. Stage App and Content Workers at 0%, predeclare stable classes, forbid ordinary lifecycle changes, and retain old bundles/images. |
