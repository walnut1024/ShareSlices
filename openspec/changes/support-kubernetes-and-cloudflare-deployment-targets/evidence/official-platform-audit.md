...
Reality recheck: 2026-07-25. See
...
3. This design chooses Durable Object `exports` for the Jobs Worker. A live Wrangler 4.112.0 spike proved that an already bootstrapped minimal exports-only Worker can use `wrangler versions upload`; the first script still requires immediate deployment, and `migrations` cannot coexist with `exports`. The full Container-bearing Jobs configuration remains unqualified for staged image rollout, Secret handling, and rollback, so the conservative default still uses immediate Jobs `wrangler deploy` with Queue delivery paused and Cron detached, followed by observed Container rollout.
...
| Worker Secrets | Supported with split release paths; deletion blocked | App and Content can upload Secrets with a zero-percent ordinary-traffic candidate. A live minimal exports spike proved that ordinary `wrangler deploy` preserves an existing encrypted Secret binding. `wrangler versions secret delete` created a non-active version but dropped `script_runtime.exports`; activation failed closed with provider code `100402`. The full Container-bearing Jobs rotation path remains unqualified. | An upgrade candidate is still selectable through an existing route with a version override, so it must already enforce production security. Preserve or update Jobs Secrets only in trigger-isolated deploy. Do not use the failed versioned deletion path. Retire a Jobs binding only after another qualified interface proves the code, configuration, `exports`, image, and retained bindings are unchanged; otherwise retain the binding and report drift. |
...
| Route-free verifier observation | Supported through separate read APIs | Script settings are read at `/script-settings`; script subdomain state reports both `workers.dev` and Preview enablement; Cron schedules are script-scoped; Custom Domains are listed account-wide; Worker Routes are listed per Zone. No single script response proves all public entry surfaces absent. | Resolve the read-only provider token only around observation, require Scripts, Custom Domains, Zone, and Workers Routes read permissions, and fail closed unless the verifier has exact Service Bindings, no schedule, disabled subdomain and Preview URLs, no account Custom Domain, and no route in every configured Zone. Keep Queue-consumer ownership in the fenced Deployment Module rather than duplicating it in Wrangler configuration. |
...
| Container image/version rollout | Partially proven | A Jobs configuration can select a Container image, but `wrangler deploy` rolls Container instances separately, normally 10% then 100%; selecting a retained bundle is not proof that instances converged. Wrangler 4.112.0 accepts version upload for an already bootstrapped minimal exports-only Worker, but that does not prove a staged Container image rollout, and gradual deployment still cannot cross a Durable Object lifecycle change. Wrangler documents JSON application and instance inventory commands; the application output includes ID, name, image, and version, while instance output includes ID, state, and application version. Provider instance IDs are runtime observations because instances are created on demand, not values available for the pre-run authorization message. | Pin and qualify the exact full Jobs toolchain and configuration. Embed a pre-publication build/release/contract identity mapped to the image digest/provider reference, exercise every production-capable stable slot, then correlate each runtime-reported instance ID against the exact checkpointed application name, image, version, and instance inventory. Reject while any selectable instance has another application version. Stage App and Content Workers at 0%, predeclare stable classes, forbid ordinary lifecycle changes, and retain old bundles/images. |
...
- [Wrangler `auth token` scoped credential retrieval](https://developers.cloudflare.com/workers/wrangler/commands/general/#auth-token)
- [Wrangler Container application and instance commands](https://developers.cloudflare.com/workers/wrangler/commands/containers/)
...
- [Worker Script Settings API](https://developers.cloudflare.com/api/resources/workers/subresources/scripts/subresources/settings/)
- [Worker Script Subdomain API](https://developers.cloudflare.com/api/resources/workers/subresources/scripts/subresources/subdomain/)
