...
After marking the nonce terminal, the Module SHALL immediately pause and detach the verifier consumer, drain or fence every nonce-scoped active invocation lease, and wait the pinned maximum in-flight Worker, Container, broker, database, and object-write interval plus its safety margin. Because a PostgreSQL pre-check is not atomic with an already-started R2 write, only a post-quiescence final inventory and idempotent cleanup MAY prove positively owned synthetic state, temporary resources, and raw evidence absent. The terminal nonce tombstone SHALL remain beyond the longer maximum configured Queue retry/retention and interrupted-recovery windows plus a pinned safety margin; its expected retention SHALL NOT be reported as an orphan or block activation. It MAY be garbage-collected only after that bound and another no-owned-state check. Unconfirmed nonce marking, quiescence, or cleanup SHALL remain a non-public isolated orphan in status and MUST NOT activate product ingress, Queue delivery, Cron, or the scheduled-execution gate.
...
The ShareSlices Jobs Worker SHALL use Durable Object `exports` for its Container classes, so its ordinary releases MUST NOT assume that `wrangler versions upload` is available. The qualified Container image SHALL be available before immediate Jobs deployment. The initial Jobs bootstrap SHALL declare every stable Container Durable Object class required by the release.
...
Terraform-owned routes, custom domains, Cron triggers, Queue consumers, Hyperdrive configurations, R2 buckets, and Queues SHALL remain at current infrastructure state and MUST be proven compatible with the previous Worker and Container contracts. Rollback MUST refuse across a Durable Object lifecycle change or when the previous version, Jobs bundle, binding, backing resource, retained Container image, selectable Container configuration, or long-lived resource compatibility is unavailable.
...
Cloudflare `verify` SHALL exercise the shared deployment contract through the production Web, API, Viewer, and isolated content addresses. It SHALL verify route ownership, bounded streaming behavior, statuses, credentials, Cookies, security headers, cache behavior, private R2 access, PostgreSQL authority, Queue duplication and recovery, bounded Container execution, graceful termination, and target-specific release metadata. Cache verification SHALL combine repeated request behavior, explicit R2/origin-read instrumentation, authorization and revocation probes, and response headers. `CF-Cache-Status` MAY corroborate the result but MUST NOT be the sole evidence of a hit or bypass.

Core ShareSlices readiness, Gallery eligibility, email capability, processing capability, and thumbnail capability SHALL be reported separately. A missing optional capability MUST fail only the product behavior that depends on it, while a release MUST NOT claim that capability ready. Gallery SHALL remain fail-closed unless the actual Cloudflare topology and all live readiness gates pass.
...
#### Scenario: Queue wake-up is duplicated during verification

- **WHEN** the contract delivers duplicate wake-up signals for one synthetic durable job
