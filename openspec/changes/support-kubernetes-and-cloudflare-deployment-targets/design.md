...
- [Hyperdrive](https://developers.cloudflare.com/hyperdrive/reference/supported-databases-and-features/) supports the repository's `pg` and Drizzle path when `nodejs_compat` and a supported compatibility date are used, but it does not support advisory locks, `LISTEN`/`NOTIFY`, SQL-level prepared-statement management, or arbitrary session state. Its cache is enabled by default and is not invalidated by writes, so this target explicitly provisions cache-disabled configurations. Current first-party pages are not consistent enough to infer origin certificate verification from a default TLS mode; qualification selects an explicit verification mode, proves hostname/certificate validation including a negative case, and distinguishes encrypted transport from authenticated origin identity.
