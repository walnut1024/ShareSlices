...
- [Worker versions and deployments](https://developers.cloudflare.com/workers/versions-and-deployments/) version code, Static Assets, bindings, and compatibility settings, but do not version R2, Queue, Durable Object, or PostgreSQL state. Wrangler 4.112.0 and the live provider accept `versions upload` for an already bootstrapped minimal Worker using Durable Object `exports`; `migrations` and `exports` are mutually exclusive. This disproves the earlier absolute incompatibility assumption. The full Jobs configuration still includes Containers and has not qualified staged image rollout, Secret handling, or rollback, so the conservative default remains immediate deployment with Queue delivery paused and Cron detached until that complete path passes the disposable-account gate.
- [Worker Secrets](https://developers.cloudflare.com/workers/configuration/secrets/) can accompany App and Content version upload through `--secrets-file`. A normal `wrangler deploy` preserves existing encrypted Secrets; the pinned minimal exports spike confirmed that preservation. Jobs rotation can supply an ephemeral `--secrets-file` during its trigger-isolated immediate deployment, but the complete Container-bearing path still requires qualification. The tested `wrangler versions secret delete` output dropped the Durable Object `exports` declaration and failed activation with provider code `100402`, so that deletion path is forbidden for Jobs.
...
The official Cache API is data-center-local, `cache.put()` does not use tiered caching, rejects `206` and no-store responses, and can reject an oversized representation. The deployment contract therefore requires the bounded `cloudflare.edgeCdn.maximumViewerAssetBytes` value even in the default mode, so switching to Viewer-byte caching never inherits an implicit runtime limit. The feasibility and cost test measures real hit ratio, R2-read reduction, and rejection behavior before enabling Viewer byte caching; the design does not claim global replication or savings merely because the API is named Cache. `CF-Cache-Status` is corroborating evidence only; repeated requests, R2-read instrumentation, and post-Unpublish or expiry probes establish correctness.
...
This design chooses Durable Object `exports` for the Jobs Worker's Container classes. The minimal exports-only spike proves that an already bootstrapped Worker can use `wrangler versions upload`, but it does not qualify the full Container-bearing Jobs bundle. Until that complete path proves staged image, Secret, activation, and rollback semantics, the qualified default does not use `wrangler versions upload`. The initial no-trigger bootstrap declares every stable Container Durable Object class, and the actual release Jobs bundle is a separate immediate deployment after bootstrap that remains trigger-isolated until the release completes.

Exact-version and route-free functional verification use a release-only harness rather than a hidden public probe endpoint. The Deployment Module creates a positively owned verifier Worker with `workers_dev` and preview URLs disabled, an isolated temporary Queue and initially delivery-paused consumer, fetch-based Service Bindings to the final App, Content, and Jobs script names, and a release-and-fence-scoped private evidence prefix. One checked message builder derives the nonce and invocation identity deterministically from the installation, operation, release, and fence, then validates the complete bounded private payload before any provider call. This determinism is recovery identity, not caller authentication; PostgreSQL lease and fence checks remain the authority. Before publishing, the Module creates the active PostgreSQL probe record only by selecting through the exact live Cloudflare deployment operation, owner, lease, release, and fencing token. An interrupted initializer may replay only the identical nonce, release, fence, sub-fence, and canonical expected Container identity; any mismatch is a conflict rather than a replacement. The Queue boundary validates the same complete message shape again before publishing, including distinct App and Content identities, exact Jobs identity, lifecycle bounds, migration head, both Container classes, unique stable slots, and image-reference agreement. It then publishes exactly one nonce plus release, fence, expected App/Content versions, expected Jobs deployment/bundle/configuration identity, and expected Container image build identity and provider reference, resumes only the temporary Queue for that bounded probe, and handles duplicate delivery as the same idempotent fenced operation. The verifier calls App and Content with the documented version-override header over `fetch()` Service Bindings and calls the route-free Jobs `fetch` probe through a checked private verification contract over a Service Binding. App and Content expose a side-effect-free `GET /health` version-evidence response only on their fixed internal Service Binding hostnames; public configured hosts continue through the ordinary product graph and cannot reach that host-qualified branch. The response reads `version_metadata`, returns `no-store`, and is persisted with the Jobs evidence before terminal fencing, so the controller can prove the override did not silently fall back even after raw invocation cleanup. Jobs evidence identifies the executing Worker version or deployment, embedded release bundle, ordinary configuration and `exports` digest, configured image reference, migration head, database access, and execution-broker scope; the controller compares all identities with the authorized release before accepting functional evidence. RPC Service Bindings are not used for version overrides, and the harness never uses production Queue, Cron, public routes, `workers.dev`, or preview URLs. The private harness protects the verification trigger and evidence; it does not make an upgrade candidate unreachable through an already attached production route.

Each Container image embeds an immutable build identity, release ID, and job or broker contract revision known before image publication. The release manifest maps that build identity to the recorded image content digest and qualified provider reference; the Container does not claim to self-measure its registry digest. Verification sequentially or within the configured concurrency bound addresses every stable trusted-processing and thumbnail slot that can receive production work. Each exercised instance returns its embedded identities plus release, fence, nonce, class, slot, and runtime-reported provider instance identity. That provider instance identity is deliberately absent from the pre-run authorization message because Cloudflare creates and replaces Container instances on demand. After terminal evidence is durable, the controller reads the exact Container application and instance inventory through Wrangler JSON mode, requires the application name, selected image, and application version to match the release checkpoint, correlates every runtime-reported instance with that inventory, and rejects any still-selectable instance on another application version. It accepts convergence only when every production-capable stable slot has exercised the expected build and no previous-image instance remains selectable. A configured image, Container listing, or one compatible exercise is insufficient evidence.

Synthetic database rows, broker attempts, Container invocations, and R2 objects use a dedicated non-product namespace positively owned by release, fence, and nonce; they never select or mutate a real account, delivery, Artifact, Version, Publication, or job. Before cleanup, the controller atomically retains the canonical terminal evidence, its digest, and the exact completing invocation identity on the nonce tombstone, marks the nonce terminal, and revokes or advances its probe sub-fence in the authoritative PostgreSQL journal. The raw invocation row may then be removed without making interrupted controller recovery depend on evidence that cleanup already deleted. Every verifier, Jobs, broker, and Container synthetic mutation rechecks the live nonce and sub-fence at its authoritative database write or commit boundary, so work that has not crossed an external side-effect boundary becomes a no-op or rejected stale write after the nonce is terminal.
...
Deployment configuration stores Secret references and operator-controlled non-secret revisions, never checked Secret values or value-derived fingerprints. Kubernetes references existing or externally synchronized Secrets and stamps only the revision onto consuming Pod templates so in-place rotation triggers the required rollout. Cloudflare uses staged version Secrets for App and Content, and trigger-isolated immediate deploy with preserved or explicitly supplied Secrets for Jobs; Terraform state never owns those values. Read-only Cloudflare provider observation uses a separate referenced API Token with only Billing Read, Zone Read, Workers Routes Read, Queues Read, Workers Scripts Read, Workers Custom Domains Read, and Workers R2 Storage Read permissions. Route-free verifier evidence reads the current `/script-settings`, script subdomain, schedules, account Custom Domains, and every configured Zone's Worker Routes; a disabled `workers.dev` endpoint alone is not route-free proof. Wrangler login proves the selected interactive account. Mutating deployment operations may retrieve the current OAuth or API Token through the official `wrangler auth token --json` command only inside one last-responsible-moment credential scope, reject Global API Keys, never emit or persist the credential, and discard it when the fenced operation returns; this is not a release-record or configuration Secret source. Resend uses a sending-access key scoped to the verified sending domain; broader provider administration is unnecessary. Release records contain resource identifiers, Secret revisions, and redacted configuration digests, not credentials, Session material, database URLs, email tokens, raw Share slugs, or object keys.
